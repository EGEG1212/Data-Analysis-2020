...
for page in range(total_pages):   #토탈페이지 아니까 여기를 추가
    ...
    for tr in trs[1:]:
        ...
        title = ''
        try:
            span = tds[1].find_element_by_tag_name('span')       #여기는 element /문제가생겨서try에넣음
            reply_count = span.string[1:-1]             #여기수정
            index = tds[1].text.find('[')  #타이틀 string=대조영[1] 에서 앞에만 가져와야해서 find('[')찾고 아래[:index]슬라이싱을 해줌
